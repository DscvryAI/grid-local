...
> = (set, get) => {
  ...
  refreshCurrentSession: async () => {
    ...
    try {
      ...
      if (selectedProject && analytics.currentView === "analytics") {
  ...
  loadProjectStatsSummary: async (projectPath: string) => {
    try {
      ...
      // These two scan the project's files independently (different
      // stats_mode, neither depends on the other's result) -- fetch them
      // concurrently instead of one after the other.
      const [billing, conversationResult] = await Promise.all([
        fetchProjectStatsSummary(projectPath, {
          ...dateOptions,
          stats_mode: "billing_total",
        }),
        canLoadConversationBreakdown()
          ? fetchProjectStatsSummary(projectPath, {
              ...dateOptions,
              stats_mode: "conversation_only",
            })
      ...
      let conversation: ProjectStatsSummary;
      if (!conversationResult.ok) {
        ...
        conversation = conversationResult.result ?? billing;
      }
