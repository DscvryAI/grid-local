...
mod tests {
    ...
    ///
    /// `HOME` alone never reached `dirs::home_dir()` on Windows -- this
    /// provider's own path resolution now calls
    /// `crate::utils::resolve_home_dir()` instead, which checks
    /// `GRID_LOCAL_HOME_OVERRIDE` first, on every platform. `HOME` is kept
    /// set too for Unix compatibility.
    struct HomeGuard {
        original: Option<String>,
        _override_guard: crate::archive_db::test_support::EnvVarGuard,
    }
    impl HomeGuard {
        fn set(path: &std::path::Path) -> Self {
            ...
            Self {
                original,
                _override_guard: crate::utils::test_support::home_override_guard(path),
            }
        }
    }
