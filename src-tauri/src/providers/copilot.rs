...
/// every sub-provider.
///
/// `PartialOrd`/`Ord` (declaration order: Cli < Desktop < `VsCode`) back
/// `merge_projects`'s deterministic `sources` sort -- see that function's
/// own doc comment for why this matters for `archive_db`'s project-key
/// idempotency.
...
fn merge_projects(parts: Vec<(SourceKind, ClaudeProject)>) -> Vec<ClaudeProject> {
    ...
    let mut merged: Vec<ClaudeProject> = grouped
        ...
        .map(|mut group| {
            ...
            // Sorted by (kind, path) -- NOT left in whatever order `group`
            // landed in after the `last_modified`-based sort above (which
            // only exists to pick a display template and is a no-op on
            // ties, so it doesn't itself guarantee determinism here).
            // `archive_db::ingest::upsert_project` treats this encoded
            // path as an exact-match idempotency key
            // (`ON CONFLICT(provider_id, project_key)`) -- an
            // undeterministic `sources` order would silently mint a NEW
            // project row on every backfill instead of updating the
            // existing one, fragmenting one real Copilot project into
            // duplicate DB rows over time. This sort makes the key stable
            // regardless of scan/filesystem walk order or how many
            // sources of the same kind a group ever ends up with.
            let mut sources: Vec<SourceRef> = group
                .iter()
                ...
                .collect();
            sources.sort_by(|a, b| a.kind.cmp(&b.kind).then_with(|| a.path.cmp(&b.path)));
...
mod tests {
    ...
    /// `archive_db::ingest::upsert_project` treats the merged `path` as an
    /// exact-match idempotency key (`ON CONFLICT(provider_id, project_key)`)
    /// -- so `merge_projects` must produce the IDENTICAL encoded key
    /// regardless of what order its caller happens to hand the same
    /// underlying sources in (a real risk in practice, since `scan_projects`
    /// builds that input from a `HashMap`-grouped, `WalkDir`/parallel-scan
    /// sourced pipeline with no inherent ordering guarantee). Feeds the
    /// SAME two sources in both orders and asserts the resulting project
    /// key is byte-identical either way.
    #[test]
    fn merge_projects_produces_a_deterministic_project_key_regardless_of_input_order() {
        let cli = project("/Users/me/repo", "copilot-cli:///Users/me/repo", 2, 50);
        let vsc = project(
            "file:///Users/me/repo",
            "vscode:///Users/me/.vscode/workspaceStorage/abc",
        ...
        let forward = merge_projects(vec![
            (SourceKind::Cli, cli.clone()),
            (SourceKind::VsCode, vsc.clone()),
        ]);
        let reversed = merge_projects(vec![(SourceKind::VsCode, vsc), (SourceKind::Cli, cli)]);

        assert_eq!(forward.len(), 1);
        assert_eq!(reversed.len(), 1);
        assert_eq!(
            forward[0].path, reversed[0].path,
            "the encoded project key must not depend on scan/input order"
        );
    }
