...
/// Same one-pass DB-open walk as [`collect_global_stats_sessions`], for
/// `commands::history`'s cross-provider session list -- but builds the
/// cheap, metadata-only [`composer_to_session`] instead of parsing message
/// bubble bodies via `stats_messages_from_composer`, since History only
/// needs session-level fields. No reliable per-session model field exists
/// in Cursor's composer data -- model stays `None`, read as "Unknown" by
/// the History surface, matching the same graceful-degradation choice
/// already accepted for Antigravity/every other Tier B/C provider.
pub fn collect_global_history_sessions() -> Result<Vec<(String, ClaudeSession)>, String> {
    let base = get_base_path().ok_or("Cursor not found")?;
    let global_db = base.join("globalStorage/state.vscdb");
    if !global_db.is_file() {
        return Ok(Vec::new());
    ...
    for row in rows.flatten() {
        let (key, value) = row;
        if key.strip_prefix("composerData:").is_none() {
            continue;
        }
        let Ok(composer) = parse_cursor_json(&value) else {
            continue;
        ...
        let workspace_id = normalize_workspace_bucket_id(&raw_workspace_id);
        let project_name = resolve_project_display_name(&ws_root, &workspace_id, &folder_path);
        if let Some(session) = composer_to_session(&composer, &project_name) {
            sessions.push((workspace_id, session));
        }
    }
