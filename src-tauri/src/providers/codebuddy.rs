    ...
    for path in candidates {
            ...
            .map_while(Result::ok)
        {
            ...
            if let Some(cwd) = value.get("cwd").and_then(|v| v.as_str()) {
                ...
                if !cwd_trimmed.is_empty() && crate::utils::looks_like_absolute_path(cwd_trimmed) {
...
mod tests {
    ...
    ///
    /// Windows-only: hyphen-encoding a Windows absolute path embeds its
    /// drive-letter colon (`C:`) as a literal, non-leading path-component
    /// character. Confirmed empirically: Windows' `CreateDirectoryW`
    /// silently accepts this (`create_dir` returns `Ok`, `Path::exists()` is even `true`
    /// via the direct path), but the resulting entry never appears in its
    /// parent's own directory listing -- `WalkDir`/`read_dir` never see it,
    /// so `scan_projects_in` finds zero sessions. This is a property of
    /// the fixture's own colon-bearing encoding scheme, not of
    /// `decode_with_filesystem_check` or `scan_projects_in` -- real
    /// Unix-style absolute paths (this test's actual target shape) never
    /// hit it, so this is gated rather than "fixed."
    #[cfg(not(windows))]
    #[test]
    fn scan_projects_falls_back_to_fs_decoding_when_cwd_missing() {
        let tmp = tempfile::tempdir().expect("tempdir");
        // Was a bare `std::fs::canonicalize()`, which returns Windows' extended-length
        // UNC form (`\\?\C:\...`) -- a shape real CodeBuddy-encoded slugs
        // never carry. Same fix `utils::tests::make_encoded_path` already
        // established, now shared via `canonicalize_for_fixture`.
        let canonical_tmp = crate::utils::test_support::canonicalize_for_fixture(tmp.path());
        ...
        // leading separator and join with `-`, mirroring how CodeBuddy
        // actually names project directories on disk -- both `/` and `\`
        // are replaced -- real paths on Windows use `\`, and the original
        // forward-slash-only replacement silently left them untouched.
