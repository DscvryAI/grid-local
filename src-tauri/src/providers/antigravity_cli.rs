...
pub(crate) fn default_root() -> Option<PathBuf> {
    crate::utils::resolve_home_dir().map(|h| h.join(".gemini").join("antigravity-cli"))
}
...
/// - `MODEL` with a `tool_calls` array → assistant, one `tool_use` block per
///   call using its real `name` (e.g. `list_dir`, `view_file`) — these steps
///   often carry no `content` at all (the model's reasoning stays in a
///   separate `thinking` field this parser never surfaces), so `tool_calls`
///   alone is enough to keep the step
/// - `MODEL` with content and no `tool_calls`: a distinctive step `type`
///   (e.g. `SEARCH_WEB`) is kept visible as a synthesized `tool_use` marker
///   before the text; `PLANNER_RESPONSE` and `GENERIC` are NOT treated as
///   tool names — `GENERIC` in particular is the step type used for a prior
///   `tool_calls` entry's own result/output being echoed back, not a tool
///   named "GENERIC" (confirmed against real transcript data: every
///   `GENERIC` step observed was tool output text, immediately following a
///   `tool_calls` step)
/// - `CONVERSATION_HISTORY` / `SYSTEM` → skip (replay/context, not turns)
/// - steps with neither content nor `tool_calls` (thinking-only) → skip
...
fn convert_step(
    ...
    if source == "MODEL" {
        ...
        if content.is_none() && tool_call_names.is_empty() {
            return None;
        }

        let mut blocks: Vec<Value> = tool_call_names
            .iter()
            .enumerate()
            .map(|(idx, name)| {
                json!({
                    "type": "tool_use",
                    "id": format!("{uuid}-tool-{idx}"),
                    "name": name,
                    "input": {}
                })
            })
        ...
        if let Some(text) = content {
            let synthesize_type_as_tool = blocks.is_empty()
                && !step_type.is_empty()
                && step_type != "PLANNER_RESPONSE"
                && step_type != "GENERIC";
            if synthesize_type_as_tool {
                blocks.push(json!({
                    "type": "tool_use",
                    "id": format!("{uuid}-tool"),
                    "name": step_type,
...
mod tests {
    ...
    #[test]
    /// Real Antigravity CLI transcripts (confirmed against a live install):
    /// a `PLANNER_RESPONSE` step carrying `tool_calls` but no `content` is
    /// how an actual tool invocation (e.g. `list_dir`) is recorded, and the
    /// `GENERIC`-typed step that follows is that tool's own result text, not
    /// a tool named "GENERIC". Both must be reflected correctly: the real
    /// tool name surfaces as a `tool_use` block, and GENERIC never fabricates
    /// a fake one.
    fn load_messages_extracts_real_tool_calls_and_does_not_fake_a_generic_tool() {
        let temp = TempDir::new().expect("temp dir");
        let root = cli_root(&temp);
        write_transcript(
            &root,
            "conv-real",
            &[
                r#"{"step_index":0,"source":"MODEL","type":"PLANNER_RESPONSE","status":"DONE","created_at":"2026-08-24T11:38:47Z","thinking":"internal reasoning, never surfaced","tool_calls":[{"name":"list_dir","args":{"DirectoryPath":"/tmp"}}]}"#,
                r#"{"step_index":1,"source":"MODEL","type":"GENERIC","status":"DONE","created_at":"2026-08-24T11:38:50Z","content":"{\"name\":\".git\",\"isDir\":true}"}"#,
                r#"{"step_index":2,"source":"MODEL","type":"PLANNER_RESPONSE","status":"DONE","created_at":"2026-08-24T11:38:51Z","tool_calls":[{"name":"view_file","args":{"AbsolutePath":"/tmp/CLAUDE.md"}}],"content":"Let me check that file."}"#,
            ],
        );
        let session_dir = root.join(BRAIN_DIR).join("conv-real");
        ...
        assert_eq!(
            messages.len(),
            3,
            "tool-call-only step must not be dropped just because it has no content"
        );

        // step 0: tool_calls-only PLANNER_RESPONSE -- real tool name, no text.
        let blocks0 = messages[0].content.as_ref().unwrap();
        assert_eq!(blocks0.as_array().unwrap().len(), 1);
        assert_eq!(blocks0[0]["type"], "tool_use");
        assert_eq!(blocks0[0]["name"], "list_dir");

        // step 1: GENERIC content is the tool's result -- plain text, never
        // a fabricated tool_use named "GENERIC".
        let blocks1 = messages[1].content.as_ref().unwrap();
        assert_eq!(
            blocks1.as_array().unwrap().len(),
            1,
            "GENERIC must not synthesize a tool_use block"
        );
        ...
        assert!(
            blocks1
                ...
                .unwrap()
                .iter()
                .all(|b| b["name"] != "GENERIC"),
            "no block may be a tool_use literally named GENERIC"
        );

        // step 2: both tool_calls and content present -- real tool name,
        // then the accompanying text, no synthesized type-based marker.
        let blocks2 = messages[2].content.as_ref().unwrap();
        assert_eq!(blocks2.as_array().unwrap().len(), 2);
        assert_eq!(blocks2[0]["type"], "tool_use");
        assert_eq!(blocks2[0]["name"], "view_file");
        assert_eq!(blocks2[1]["type"], "text");
        assert_eq!(blocks2[1]["text"], "Let me check that file.");
    }
