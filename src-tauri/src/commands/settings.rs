...
//! user settings presets stored in ~/.grid-local/presets/
...
mod tests {
    ...
    /// `env::set_var("HOME", ...)` alone never worked on Windows -- kept
    /// for Unix compatibility, paired with `home_override_guard`, which
    /// `crate::utils::resolve_home_dir()` checks first on every platform.
