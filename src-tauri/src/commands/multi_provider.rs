...
use crate::models::{ClaudeMessage, ClaudeProject, ClaudeSession, MessagePage, ProviderTier};
use crate::providers;
...
/// Coverage tier for every provider `archive_db` has ever ingested -- see
/// `archive_db::list_provider_tiers`'s own doc for what "tier" means and
/// its one caveat (a provider never ingested has no row here).
#[tauri::command]
pub async fn get_provider_tiers() -> Result<Vec<ProviderTier>, String> {
    let conn = archive_db::open_connection()?;
    archive_db::list_provider_tiers(&conn)
}
