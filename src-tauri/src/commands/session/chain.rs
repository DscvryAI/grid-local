...
pub(crate) type ProjectSnapshot = Vec<(PathBuf, FileSignature)>;
...
/// `pub(crate)` so callers that resolve chains for MANY sessions in the
/// same project in one pass (`superseded_chain_paths` below, and
/// `archive_db::ingest`'s per-session loop via `load_session_messages_
/// with_snapshot`) can compute this ONCE and share it explicitly, instead
/// of every call recomputing its own O(n) directory scan -- see
/// `resolve_session_chain_with_snapshot`'s doc comment for the real,
/// confirmed O(n²) bug this closes.
/// A time-based cache was tried first and reverted: it broke
/// `invalidates_cache_when_a_missing_predecessor_appears` by making a
/// fast-running test observe a stale snapshot within its own cache
/// window -- explicit sharing has no such staleness window at all.
pub(crate) fn project_snapshot(project_dir: &Path) -> Option<ProjectSnapshot> {
...
pub fn resolve_session_chain(session_path: &Path) -> Vec<PathBuf> {
    let snapshot = session_path.parent().and_then(project_snapshot);
    resolve_session_chain_with_snapshot(session_path, snapshot.as_ref())
}

/// Same as [`resolve_session_chain`], but takes an already-computed project
/// snapshot instead of recomputing one internally.
///
/// Splitting this out matters for callers that resolve chains for EVERY file
/// in a project in one pass (`superseded_chain_paths`, below): the plain
/// per-call version recomputes the snapshot (a full directory scan) twice per
/// call, which is a genuine, previously-accepted cost for a single ad hoc
/// resolution (see the doc comment above `SESSION_CHAIN_CACHE`) but becomes
/// O(n²) -- confirmed empirically to hang for 2+ hours at 10,000 session
/// files when called once per file across a whole project. Computing the
/// snapshot ONCE and sharing it across
/// every file's resolution turns that into O(n).
pub(crate) fn resolve_session_chain_with_snapshot(
    session_path: &Path,
    ...
    if let Some(snapshot) = snapshot {
        SESSION_CHAIN_CACHE
            ...
            .insert(
                key,
                CachedChain {
                    project_snapshot: snapshot.clone(),
                    chain: chain.clone(),
                },
            );
    }
...
pub fn superseded_chain_paths(project_root: &Path) -> HashSet<PathBuf> {
    ...
    // Computed ONCE and shared across every file's resolution below --
    // see `resolve_session_chain_with_snapshot`'s own doc comment for why
    // this specifically (not the single-session `resolve_session_chain`)
    // was the real O(n²) cost behind the 10,000-session archive_db-ingest
    // hang: `ingest_claude_project` -> `load_project_sessions` -> this
    // function, once per project scan.
    ...
    for entry in entries.filter_map(Result::ok) {
        ...
        let chain = resolve_session_chain_with_snapshot(&path, snapshot.as_ref());
        if chain.len() > 1 {
