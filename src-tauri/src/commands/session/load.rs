...
/// Stable, non-cryptographic digest of a project path, used only to derive a
/// filesystem-safe cache filename -- not a security boundary.
...
/// Directory for Grid's own session-metadata cache files: `~/.grid-local/session-cache/`.
/// Deliberately kept OUTSIDE any provider's own storage directory -- Grid must
/// never write into a provider's directory (read-only guarantee, spec §22).
/// `pub(crate)` so Settings' "Delete Grid's local data" (spec §31) can clear
/// it without duplicating this path-join logic a second time.
pub(crate) fn get_session_cache_dir() -> Option<PathBuf> {
    let home = crate::utils::resolve_home_dir()?;
    Some(home.join(".grid-local").join("session-cache"))
}

/// Get the cache file path for a project. `None` if the home directory can't
/// be resolved -- `load_cache`/`save_cache` treat that as a silent cache-miss
/// / best-effort no-op, matching this cache's existing failure semantics.
fn get_cache_path(project_path: &str) -> Option<PathBuf> {
    let dir = get_session_cache_dir()?;
    Some(dir.join(format!("{}.session_cache.json", hash_project_path(project_path))))
}

/// Load cache from disk
fn load_cache(project_path: &str) -> SessionMetadataCache {
    let Some(cache_path) = get_cache_path(project_path) else {
...
pub async fn load_session_messages(session_path: String) -> Result<Vec<ClaudeMessage>, String> {
    load_session_messages_with_snapshot(session_path, None).await
}

/// Same as [`load_session_messages`], but takes an already-computed project
/// snapshot instead of letting `resolve_session_chain` recompute one
/// internally.
///
/// Splitting this out matters for callers that load full message content
/// for MANY sessions in the same project in one pass
/// (`archive_db::ingest_claude_project`'s per-session loop): the plain
/// per-call version recomputes the snapshot (a full directory scan) on
/// every call, which is a genuine, previously-accepted cost for a single ad
/// hoc load (see `chain.rs`'s own `SESSION_CHAIN_CACHE` doc comment) but
/// becomes O(n²) -- confirmed empirically to still take ~10 minutes at
/// 10,000 sessions even after the OTHER O(n²) ingest bottleneck
/// (`superseded_chain_paths`) was separately fixed -- when called once
/// per session across a whole project. Computing the snapshot ONCE and
/// sharing it across every
/// session's load turns that into O(n).
// Genuinely does no async work today (every step is synchronous file I/O),
// but stays `async` to match every call site's existing `.await` usage
// (`load_session_messages` itself, plus `ingest_claude_session_file_with_
// snapshot`) -- removing `async` here would cascade into removing it from
// those too, a larger and riskier change than this one function's own
// scope calls for.
#[allow(clippy::unused_async)]
pub(crate) async fn load_session_messages_with_snapshot(
    session_path: String,
    snapshot: Option<&super::chain::ProjectSnapshot>,
    ...
    let chain = super::chain::resolve_session_chain_with_snapshot(Path::new(&session_path), snapshot);
...
mod tests {
    ...
    /// Run `body` with `$HOME` temporarily pointed at `home`, restoring it
    /// afterward even if `body` panics (an env var left mutated by a
    /// panicked test would otherwise corrupt every later test in this
    /// process that resolves the home dir). Serialized because the session
    /// cache resolves the home dir from the process environment (other
    /// suites also override `HOME`).
    #[cfg(unix)]
    ...
    /// Regression test for a real, confirmed O(n²) bug:
    /// `superseded_chain_paths` used to call
    /// `resolve_session_chain` once per session file, and that function
    /// recomputed a full project directory snapshot (a `fs::read_dir` +
    /// per-file `stat`) TWICE on every call -- fine for the single-session
    /// call sites, catastrophic when called once per file across a whole
    /// project. Confirmed via a live repro: 10,000 sessions took over 2
    /// hours (never finished; killed) before the fix, and 56-162 seconds
    /// after it across repeated runs under varying system load
    /// (`resolve_session_chain_with_snapshot`, `chain.rs`, computes the
    /// snapshot ONCE and shares it across every file's resolution). This
    /// asserts a generous bound (5 minutes for 10,000 sessions in a debug
    /// build) that the old O(n²) behavior would blow through by orders of
    /// magnitude, without pinning to this machine's exact real number or
    /// making the test flaky under normal load variance.
    /// `#[ignore]`d (writing + parsing 10,000 files is legitimately slow
    /// for routine `cargo test` runs) -- run explicitly with `cargo test
    /// --release -- --ignored load_project_sessions_stays_near_linear_at_
    /// scale --nocapture`.
    #[tokio::test]
    #[ignore = "slow -- run explicitly to guard the O(n^2) chain-resolution fix"]
    async fn load_project_sessions_stays_near_linear_at_scale() {
        ...
        assert!(
            elapsed.as_secs() < 300,
            "load_project_sessions({N}) took {elapsed:?} -- the old O(n^2) chain-resolution \
             bug took over 2 HOURS at this scale, so any regression back toward it should \
             blow through this generous bound long before hitting a real timeout"
        );
    }
...
"#;
    ...
    async fn test_load_project_sessions_prefers_jsonl_cwd_for_project_name() {
        ...
        // `cwd` must go through `serde_json::to_string` (not raw
        // interpolation) so a Windows path's backslashes are JSON-escaped
        // -- a bare `\U`/`\A`/`\T`... isn't a valid JSON escape, and the
        // unescaped form only ever parsed by accident on Unix, where paths
        // never contain backslashes.
        let content = format!(
            r#"{{"uuid":"uuid-1","sessionId":"session-1","timestamp":"2025-06-26T10:00:00Z","type":"user","cwd":{},"message":{{"role":"user","content":"Hello world"}}}}
"#,
            serde_json::to_string(&actual_cwd).unwrap()
        );
    ...
    #[tokio::test]
    #[cfg(not(windows))]
    // Hardcodes `/usr/lib`, which has no Windows equivalent -- a genuine
    // platform-specific assertion, same class as
    // `test_decode_project_path_verified_resolves_existing_folder` in
    // `utils.rs`.
    ...
    async fn test_load_project_sessions_page_total_stays_cumulative_without_disk_cache() {
        ...
        // Restores $HOME on drop, even if an assertion below panics -- an
        // env var left mutated by a panicked test would otherwise corrupt
        // every later test in this process that resolves the home dir.
        struct RestoreHome(Option<std::ffi::OsString>);
        impl Drop for RestoreHome {
            fn drop(&mut self) {
                match self.0.take() {
        ...
        // registry, so `total` stays consistent on every page. The cache now
        // lives under `~/.grid-local/session-cache/` (never inside the
        // project directory -- read-only guarantee), keyed by a hash of the
        // project path. Ask the code under test where it put the file
        // rather than assuming `$HOME` mocking is honored identically on
        // every platform (see the Windows `dirs::home_dir()` caveat noted
        // elsewhere in this codebase).
        let cache_path = get_cache_path(&project_path)
            .expect("cache path should resolve once HOME is set for this test");
...
"#;
    ...
    // Regression coverage for the read-only-guarantee fix: the session
    // metadata cache used to live at `<project_path>/.session_cache.json`,
    // writing Grid's own cache file directly into the provider's real
    // storage directory. It must now live under Grid's own
    // `~/.grid-local/session-cache/`, never under any provider path.

    // Gated to unix, matching this codebase's established precedent for
    // tests that assert an exact filesystem location derived from a mocked
    // `$HOME` (see `commands::session::mod`'s own `with_home`-based tests,
    // similarly unix-only) -- `dirs::home_dir()`'s `$HOME`-override behavior
    // on Windows is not reliably exercised by this pattern.
    #[cfg(unix)]
    #[test]
    #[serial]
    fn session_cache_path_lands_under_grid_local_not_the_project_dir() {
        let home = TempDir::new().unwrap();
        let provider_project_dir = home.path().join(".claude").join("projects").join("proj");
        std::fs::create_dir_all(&provider_project_dir).unwrap();
        let project_path = provider_project_dir.to_string_lossy().to_string();

        let cache_path = with_home(home.path(), || get_cache_path(&project_path)).unwrap();

        assert!(
            cache_path.starts_with(home.path().join(".grid-local")),
            "cache path {} must live under ~/.grid-local",
            cache_path.display()
        );
        assert!(
            !cache_path.starts_with(&provider_project_dir),
            "cache path {} must not live inside the provider's own project directory",
            cache_path.display()
        );
    }
    ...
    #[test]
    #[serial]
    fn session_cache_save_and_load_round_trips_under_grid_local() {
        let home = TempDir::new().unwrap();
        let provider_project_dir = home.path().join(".claude").join("projects").join("proj");
        std::fs::create_dir_all(&provider_project_dir).unwrap();
        let project_path = provider_project_dir.to_string_lossy().to_string();
        ...
        with_home(home.path(), || {
            ...
            // Nothing should have been written inside the provider's own
            // project directory.
            assert!(
                !provider_project_dir.join(".session_cache.json").exists(),
                "the old in-project cache path must not be written"
            );

            let reloaded = load_cache(&project_path);
            assert_eq!(reloaded.version, CACHE_VERSION);
            assert!(reloaded.entries.contains_key("some-session.jsonl"));
        });
    }
}
