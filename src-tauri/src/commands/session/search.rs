...
pub async fn search_messages(
    ...
    // This fallback path accepted a caller-supplied `limit` with no
    // upper bound, unlike `search_archive_fts`/`get_git_log` (already
    // clamped) -- shares that same maximum now.
    let max_results = limit
        .unwrap_or(DEFAULT_SEARCH_LIMIT)
        .clamp(1, crate::commands::search::MAX_SEARCH_RESULT_LIMIT);
...
mod tests {
    ...
    /// A caller-supplied `limit` previously passed straight through with
    /// no upper bound, unlike
    /// `search_archive_fts`/`get_git_log` (already clamped). A huge
    /// requested limit must still be capped at
    /// `commands::search::MAX_SEARCH_RESULT_LIMIT` (500), not returned
    /// as-is.
    #[tokio::test]
    async fn test_search_messages_caps_a_huge_caller_supplied_limit() {
        let temp_dir = TempDir::new().unwrap();
        let projects_dir = temp_dir.path().join("projects");
        let project_dir = projects_dir.join("test-project");
        std::fs::create_dir_all(&project_dir).unwrap();
        ...
        for i in 0..600 {
            content.push_str(&create_sample_user_message(
                &format!("uuid-{i}"),
                &format!("session-{i}"),
                "needle content",
            ));
        ...
        assert_eq!(
            messages.len(),
            crate::commands::search::MAX_SEARCH_RESULT_LIMIT,
            "an unbounded caller-supplied limit must still be capped"
        );
    }
