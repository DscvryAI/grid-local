...
pub async fn send_feedback(feedback: FeedbackData) -> Result<(), String> {
    ...
    let feedback_email = std::env::var("FEEDBACK_EMAIL")
        .unwrap_or_else(|_| "grid-local@dscvryai.com".to_string());
    let mailto_url =
...
/// Third-party attribution notices for Settings' "About > Licences" (spec
/// §31). Embedded at COMPILE TIME via `include_str!` -- the file is a
/// repo-root build artifact, not something guaranteed to exist on disk
/// next to a shipped binary, so a runtime path read would be fragile.
#[tauri::command]
pub async fn get_third_party_notices() -> Result<String, String> {
...
pub async fn open_github_issues(feedback: Option<FeedbackData>) -> Result<(), String> {
    // TODO(grid): placeholder until the real Grid Local repo exists.
    let base_url = "https://github.com/dscvry/grid-local/issues/new";
