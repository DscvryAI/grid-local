...
//! user metadata stored in ~/.grid-local/user-data.json
...
pub(crate) fn validate_project_metadata_key(project_path: &str) -> Result<(), String> {
    ...
    if !crate::utils::looks_like_absolute_path(project_path) {
        return Err(format!(
...
/// Get the metadata folder path (~/.grid-local)
...
/// Get the user data file path (~/.grid-local/user-data.json)
...
mod tests {
    ...
    /// Returns the mutex guard, the `TempDir`, and a `GRID_LOCAL_HOME_OVERRIDE`
    /// guard -- `env::set_var("HOME", ...)` alone never worked on Windows,
    /// kept here for Unix compatibility only. All three must be kept alive
    /// for the duration of the test.
    fn setup_test_env() -> (
        MutexGuard<'static, ()>,
        TempDir,
        crate::archive_db::test_support::EnvVarGuard,
    ) {
        let guard = TEST_ENV_MUTEX.lock().unwrap();
        let temp_dir = TempDir::new().unwrap();
        env::set_var("HOME", temp_dir.path());
        let home_guard = crate::utils::test_support::home_override_guard(temp_dir.path());
        (guard, temp_dir, home_guard)
    }

    #[test]
    fn test_get_metadata_folder() {
        let (_guard, _temp, _home_guard) = setup_test_env();
        let folder = get_metadata_folder().unwrap();
        assert!(folder.to_string_lossy().contains(".grid-local"));
    }
    ...
    fn test_atomic_write() {
        let (_guard, temp, _home_guard) = setup_test_env();

        // Manually create the metadata folder since HOME is mocked
        let metadata_folder = temp.path().join(".grid-local");
