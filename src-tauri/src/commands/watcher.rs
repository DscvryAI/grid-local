...
fn handle_file_event(app_handle: &AppHandle, event: &DebouncedEvent) {
    ...
    trigger_archive_reingest(&event.path);
}

/// Returns the Claude project directory path if `path` is a Claude Code
/// session file (the same shape `extract_paths` recognizes), else `None`.
/// Split out from [`trigger_archive_reingest`] purely so this detection
/// logic is unit-testable without opening a real database.
fn claude_project_path_for_watch_event(path: &Path) -> Option<String> {
    extract_paths(path).map(|(project_path, _session_path)| {
        project_path.to_string_lossy().to_string()
    })
}

/// Fire-and-forget: if `path` is a Claude Code session file, incrementally
/// re-ingests just that one project into Grid's own normalized archive
/// (Phase 0's `archive_db`, spec §20-21) in the background -- no global
/// directory walk, no re-scan of every project, just the one project this
/// change belongs to (whose own already-ingested sessions are skipped
/// cheaply via the `(size, mtime, parser_version)` check in
/// `archive_db::ingest`). Failures are logged only; this must never affect
/// the live-reload event emitted above, which existing UI behavior
/// depends on.
fn trigger_archive_reingest(path: &Path) {
    let Some(project_path) = claude_project_path_for_watch_event(path) else {
        return;
    };
    tauri::async_runtime::spawn(async move {
        let mut conn = match crate::archive_db::open_connection() {
            Ok(conn) => conn,
            Err(e) => {
                log::error!("Failed to open Grid archive for incremental re-ingest: {e}");
                return;
            }
        };
        if let Err(e) =
            crate::archive_db::ingest::ingest_claude_project(&mut conn, &project_path).await
        {
            log::error!("Incremental Grid archive re-ingest failed for `{project_path}`: {e}");
        }
    });
}
...
mod tests {
    ...
    #[test]
    fn claude_project_path_for_watch_event_matches_claude_session_files() {
        let path = PathBuf::from("/Users/test/.claude/projects/my-project/session.jsonl");
        let result = claude_project_path_for_watch_event(&path);
        assert!(result.is_some());
        // Compare as a `Path` (component-aware, separator-agnostic) rather
        // than a raw string suffix, since `to_string_lossy()` renders
        // native separators (`\` on Windows).
        assert!(PathBuf::from(result.unwrap()).ends_with("projects/my-project"));
    }

    #[test]
    fn claude_project_path_for_watch_event_ignores_non_claude_paths() {
        // A Codex rollout path -- shares the .jsonl extension with Claude
        // but a different directory shape, so it must not trigger a
        // Claude-archive re-ingest.
        let path = PathBuf::from("/Users/test/.codex/sessions/2025/10/rollout-abc.jsonl");
        assert!(claude_project_path_for_watch_event(&path).is_none());
    }
