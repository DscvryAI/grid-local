...
//! cold scan itself is already nondeterministic for them. The day-bucket
//! machinery above (`build_message_file_aggregate`/`compose_project`/
//! `compose_global`) is specific to Claude's raw JSONL row format and isn't
//! reused for other providers.
//!
//! ## Generic-provider global stats cache
//!
//! [`provider_session_stats_cache`] is a separate, much simpler cache for
//! `get_global_stats_summary`'s generic (non-Claude, non-Cursor,
//! non-Antigravity) provider path, which previously had **no** caching at
//! all — every session's messages were reloaded and re-aggregated into a
//! `SessionFileStats` from scratch on every single call. Confirmed on a real
//! machine: one Codex project's 392 sessions took 46+ seconds this way,
//! dwarfing every other provider combined, and re-parallelizing the scan
//! made no measurable difference (the per-file mmap+parse cost itself is
//! the bottleneck, not serialization).
//!
//! Unlike the day-bucket cache above, this stores the **whole already-
//! computed** `SessionFileStats` per `(file_path, mode)`, validated by
//! `(size, mtime)` the same way — no day-bucket composition, so it's only
//! consulted for the **unfiltered** (no start/end date) case; a date-filtered
//! call always rebuilds. It degrades safely for providers whose session path
//! is a virtual scheme URI with no real backing file (`opencode://` …):
//! `fs::canonicalize`/`fs::metadata` simply fail and the caller falls back to
//! an uncached build, exactly as before this cache existed. It only actually
//! helps providers backed by a real file, like Codex's rollout JSONL.
...
struct ProviderSessionStatsEntry {
    size: u64,
    mtime: Option<SystemTime>,
    /// One slot per `StatsMode`, same indexing as [`mode_slot`].
    slots: [Option<Arc<SessionFileStats>>; 2],
}

/// Whole-result cache for `get_global_stats_summary`'s generic
/// (non-Claude/Cursor/Antigravity) provider path. See the module-level
/// "Generic-provider global stats cache" doc above for why this exists and
/// why it's a different shape from [`StatsFileCache`].
pub(super) struct ProviderSessionStatsCache {
    entries: Mutex<HashMap<PathBuf, ProviderSessionStatsEntry>>,
}

static PROVIDER_SESSION_STATS_CACHE: OnceLock<ProviderSessionStatsCache> = OnceLock::new();

pub(super) fn provider_session_stats_cache() -> &'static ProviderSessionStatsCache {
    PROVIDER_SESSION_STATS_CACHE.get_or_init(ProviderSessionStatsCache::new)
}

impl ProviderSessionStatsCache {
    fn new() -> Self {
        Self {
            entries: Mutex::new(HashMap::new()),
        }
    ...
    /// Return the cached stats for `path`/`mode`, rebuilding via `build` when
    /// the entry is missing, the file's `(size, mtime)` changed, or `path`
    /// can't be stat'ed at all (e.g. a provider's virtual scheme path with no
    /// real backing file -- always rebuilds, uncached, in that case).
    pub(super) fn get_or_build<F>(
        &self,
        path: &Path,
        mode: StatsMode,
    ...
    where
        F: FnOnce() -> Option<SessionFileStats>,
    {
        let Ok(key) = fs::canonicalize(path) else {
            return build();
        };
...

        {
            ...
            if let Some(entry) = entries.get(&key) {
                if entry.size == size && entry.mtime == mtime {
                    if let Some(stats) = &entry.slots[slot] {
        ...
        let built = build()?;
        let arc_built = Arc::new(built.clone());
        ...
        match entries.get_mut(&key) {
            Some(entry) => {
                ...
                entry.slots[slot] = Some(arc_built);
            }
            None => {
                // Memory bound: past the cap new files are computed but not
                // stored, keeping already-hot entries intact (matches
                // StatsFileCache's own cap behavior).
                if entries.len() < MAX_CACHE_ENTRIES {
                    let mut slots: [Option<Arc<SessionFileStats>>; 2] = [None, None];
                    slots[slot] = Some(arc_built);
                    entries.insert(
                        key,
                        ProviderSessionStatsEntry {
                            size,
                            mtime,
                            slots,
                        },
    ...
    fn lock_entries(&self) -> std::sync::MutexGuard<'_, HashMap<PathBuf, ProviderSessionStatsEntry>> {
        match self.entries.lock() {
            Ok(guard) => guard,
            Err(poisoned) => poisoned.into_inner(),
        }
