...
//! Location: ~/.grid-local/user-data.json
...
pub struct UserSettings {
    ...
    /// Whether the user has been shown the first-run "scan for other AI
    /// tools" prompt (accepted or skipped) — gates showing it again.
    #[serde(default)]
    pub has_seen_provider_discovery_prompt: bool,

    /// Archive sync preference: `"auto"` (re-run `sync_grid_index` on every
    /// app launch — idempotent, cheap after the first run) or `"manual"`
    /// (only via an explicit Settings action). `None` = not yet decided.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub archive_sync_mode: Option<String>,

    /// Whether the user has been shown the one-time "keep your archive up
    /// to date?" prompt (picked a mode, or dismissed it — dismissing
    /// implies `"manual"`) — gates showing it again.
    #[serde(default)]
    pub has_seen_archive_sync_prompt: bool,
}
