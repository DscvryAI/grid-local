...
/// - `grid-local://session/<uuid>` — becomes a `Uuid` hint
/// - `grid-local://session-folder/<name>` — becomes a `Folder` hint
/// - `grid-local://session-title/<url-encoded-text>` — becomes a `Title` hint
///
...
mod tests {
    ...
    fn parses_custom_scheme_uuid_url() {
        let hint = parse_session_hint_from_url(&url(
            "grid-local://session/1265cd74-caa9-472e-b343-c4f44b5cf12c",
        ))
        .expect("hint");
        assert_eq!(hint.kind, SessionHintKind::Uuid);
        assert_eq!(hint.value, "1265cd74-caa9-472e-b343-c4f44b5cf12c");
    ...
    fn parses_custom_scheme_folder_url() {
        let hint =
            parse_session_hint_from_url(&url("grid-local://session-folder/demo"))
                .expect("hint");
        assert_eq!(hint.kind, SessionHintKind::Folder);
        assert_eq!(hint.value, "demo");
    }
    ...
    fn parses_custom_scheme_title_with_spaces() {
        let hint = parse_session_hint_from_url(&url(
            "grid-local://session-title/auth%20bug",
        ))
        .expect("hint");
        assert_eq!(hint.kind, SessionHintKind::Title);
        assert_eq!(hint.value, "auth bug");
    ...
    fn rejects_custom_scheme_with_bad_uuid_host() {
        let hint =
            parse_session_hint_from_url(&url("grid-local://session/not-a-uuid"));
        assert!(hint.is_none());
    }

    #[test]
    fn rejects_custom_scheme_with_unknown_host() {
        let hint = parse_session_hint_from_url(&url("grid-local://unknown/demo"));
        assert!(hint.is_none());
    }
    ...
    fn rejects_custom_scheme_with_empty_path() {
        let hint =
            parse_session_hint_from_url(&url("grid-local://session-folder/"));
        assert!(hint.is_none());
    }
    ...
    fn title_survives_utf8_multibyte_round_trip() {
        ...
        let hint = parse_session_hint_from_url(&url(
            "grid-local://session-title/%ED%95%9C%EA%B8%80%20%EC%A0%9C%EB%AA%A9",
        ))
        .expect("hint");
        assert_eq!(hint.kind, SessionHintKind::Title);
    ...
    fn title_preserves_literal_plus_character() {
        ...
        let hint = parse_session_hint_from_url(&url(
            "grid-local://session-title/C%2B%2B%20bug",
        ))
        .expect("hint");
    ...
    fn title_plus_is_not_decoded_to_space() {
        ...
        let hint =
            parse_session_hint_from_url(&url("grid-local://session-title/one+two"))
                .expect("hint");
