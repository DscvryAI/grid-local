...
describe('Tauri Configuration Tests', () => {
  ...
  describe('Product Information Validation', () => {
    it('should have correct product name', () => {
      expect(config.productName).toBe('Grid Local');
    ...
    it('should have valid app identifier in reverse domain notation', () => {
      ...
      expect(config.identifier).toBe('com.dscvry.grid-local');
  ...
  describe('Security Configuration Validation', () => {
    ...
    it('should have zero remote origins in the CSP (spec §24: no remote fonts, no remote images, zero outbound traffic in normal use)', () => {
      const csp = config.app.security.csp;
      expect(typeof csp).toBe('string');

      // Every directive value must be 'self', 'unsafe-inline', a data: URI,
      // or 'none' -- no http(s) origin of any kind. This is a real
      // regression test for a real incident: this CSP previously allowed
      // fonts.googleapis.com, fonts.gstatic.com, github.com, and any
      // https: image, none of which are needed once fonts are bundled
      // locally (see src/main.tsx's @fontsource imports).
      expect(csp).not.toMatch(/https?:\/\//);

      // Belt-and-suspenders: name the specific hosts this app has
      // historically leaked into its CSP, so a future PR that
      // re-adds one of them by copy-pasting an old snippet fails loudly
      // with a clear reason instead of a generic regex mismatch.
      const bannedHosts = [
  ...
  describe('Plugins Configuration Validation', () => {
    ...
    describe('Updater Plugin', () => {
      // Grid Local deliberately ships with the updater plugin INACTIVE and
      // no endpoints/pubkey configured: there's no signing infrastructure
      // yet, and upstream's config pointed at their own signed release
      // feed, which would have had Grid silently check (and potentially
      // install) upstream's builds. Do not flip `active` back to true
      // here without also confirming real signing keys + endpoints + an
      // explicit (never automatic-by-default) check UI exist.
      it('should have the updater plugin present but inactive (no release infra yet)', () => {
      ...
      it('should have no update endpoints configured', () => {
      ...
      it('should have no pubkey configured (no Dscvry signing keys yet)', () => {
  ...
  describe('Bundle Configuration Validation', () => {
    it('should have bundle configuration enabled', () => {
      ...
      // The updater plugin is disabled (no signing keys exist yet), so
      // producing signed updater artifacts here would fail the build.
  ...
  describe('Security and Privacy Validation', () => {
    ...
    it('should have proper reverse domain notation identifier', () => {
      ...
      expect(parts[1]).toBe('dscvry');
      expect(parts[2]).toBe('grid-local');
    });
    ...
    it('should not expose private info in the (currently empty) updater pubkey field', () => {
      expect(config.plugins.updater.pubkey).toBeDefined();
      expect(typeof config.plugins.updater.pubkey).toBe('string');
      // No Dscvry signing keys exist yet — pubkey is intentionally empty,
      // not a real key. See "should have no pubkey configured" above.
